...
export const handleOrderTasks = (tasks: Map<string, Task>): Promise<{ orderedTasks: string[]; tasksDegree: string[][] }> => {
  return new Promise((resolve, reject) => {
    ...
    let inDegreeCopy = new Map(inDegree);
    let graphCopy: Map<string, string[]> = new Map(graph);
    ...
    resolve({ orderedTasks, tasksDegree });
  });
}